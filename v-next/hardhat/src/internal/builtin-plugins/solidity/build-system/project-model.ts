...
const importQuery = `(
     ...
     | [NamedImport @namedPath path: [_]]
     | [ImportDeconstruction @deconstructedPath path: [_]]
     )`;
const pragmaQuery = `[VersionPragma [VersionExpressionSets (@versionExpression [VersionExpression])+]]`;
const queries = [importQuery, pragmaQuery].map((q) => Query.parse(q));

export type SourceName = string;
export type Version = string; // TODO: find something better, from semver most likely
